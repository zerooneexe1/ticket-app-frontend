const API_URL = 'https://ticket-app-backend-production-c69a.up.railway.app/api/auth';
...
export async function registerWithEmail(email, password, name) {
    try {
        ...
        return true; // Register sukses, lanjut ke verifikasi OTP
    } catch (error) {
        window.showToast(error.message, "error");
        return false;
    }
...
export async function verifyEmailOTP(email, otp) {
    try {
        ...
        window.showToast(data.message, "success");
        return true; // Verifikasi sukses, lanjut ke login
    } catch (error) {
        window.showToast(error.message, "error");
        return false;
    }
...
// Cek status login (menggantikan onAuthStateChanged)
export function initAuth(callback) {
    ...
    if (token && userStr) {
        ...
        // Kalau rolenya admin, set variable isAdmin menjadi true
        const isAdmin = userData.role === 'admin'; 
        
        // Kita membungkus 'user' dengan objek agar struktur callback di index.html tidak rusak
